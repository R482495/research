...
class _HearableServiceViewState extends State<_HearableServiceView> {
  ...
  bool _isTripleShakeDetected = false;
  //頭の振りリセット用
  Timer? _resetTimer;
  ...
  List<List<String>> accData = [
    ["time", "ay", "az", "gx", "gy", "gz", "maccyz", "mgxyz"]
  ];
  ...
  void addAccelData(List<String> accelDataBatch) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      //added--------------------
      double maccyz = sqrt(ay * ay + az * az);
                ...
                Consumer<NineAxisSensor>(
                  ...
                  if (nineAxisSensor.isEnabled) {
                    ...
                    if (accelDataBatch.length >= 6) {
                      ...
                      if (isShaking && !_isTripleShakeDetected) {
                        setState(() {
                          _isTripleShakeDetected = true;
                        });
