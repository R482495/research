...
class Temperature extends ChangeNotifier {
  ...
  //added-----------------------------------------------
  String getTemperatureDataBatch() {
    String temp = '';
    String res = '';
    if (_resultCode != null) {
      temp += 'result code: $_resultCode';
    }
    if (_data != null) {
      temp += '\nbyte[]:\n';
      Uint8List data = _data!;
      for (int i = 0; i < data.length - 1; i++) {
        temp += '${data[i].toRadixString(16)}, ';
      }
      temp += '${data.last.toRadixString(16)}\n';
      Uint8List tempByte = Uint8List.fromList(data.sublist(0, 2));
      var bytes = tempByte.buffer.asByteData();
      res += '${bytes.getInt16(0, Endian.little) / 10.0}';
      temp += res;
    }
    return res;
  }
